...
def dedupe_latest(events: Iterable[IngestionEvent]) -> list[IngestionEvent]:
    ...
    for event in events:
        ...
        if key not in latest_by_key:
            ...
            current_latest = latest_by_key[key]
            if (event.occurred_at, event.event_id) > (
                current_latest.occurred_at,
                current_latest.event_id,
            ):
                latest_by_key[key] = event
...
def readiness_status(probes: Iterable[dict[str, Any]]) -> str:
    ...
    for probe in probes:
        is_healthy = probe.get("ready" , probe.get("healthy", False))
        is_mandatory = probe.get("mandatory", True)
